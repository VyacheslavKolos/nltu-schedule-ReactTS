import {IGroup} from "../../interfaces";

export const NumeratorLessonsSchedule_KN_31_2: IGroup =
    {
        group: 'КН-31/2',
        isNumerator:true,
        schedule: [{
            time: '8:30',
            lessons: [
                {
                    day: 'Понеділок',
                    lessonInfo: null
                },
                {
                    day: 'Вівторок',
                    lessonInfo: null

                },
                {
                    day: 'Cереда',
                    lessonInfo: null
                },
                {
                    day: 'Четвер',
                    lessonInfo: null
                },
                {
                    day: 'П\'ятниця',
                    lessonInfo: null
                },
            ],
        },
            {
                time: '10:20',
                lessons: [
                    {
                        day: 'Понеділок',
                        lessonInfo: null
                    },
                    {
                        day: 'Вівторок',
                        lessonInfo: {
                            name: "Методи та засоби ООАП",
                            teacher: 'Соколовський Я.І.',
                            link: 'https://us02web.zoom.us/j/8894953167?pwd=dmZQNW9zR1lVV3pSaU5wd1RWVUgyUT09',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Cереда',
                        lessonInfo: {
                            name: "Веб-технології та веб-дизайн",
                            teacher: 'Яркун В.І.',
                            link: 'https://meet.google.com/tkb-nsfk-bvs',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'Четвер',
                        lessonInfo: {
                            name: "ШНМ",
                            teacher: 'Розумовський М.П.',
                            link: 'https://meet.google.com/beb-kffp-pif',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'П\'ятниця',
                        lessonInfo:  {
                            name: 'Програмув. мобільних систем',
                            teacher: 'Івасюк Р.В.',
                            link: 'https://meet.google.com/yar-vkvb-vqm',
                            type: 'Лабораторне'
                        }
                    },
                ],
            },
            {
                time: '12:10',
                lessons: [
                    {
                        day: 'Понеділок',
                        lessonInfo:  {
                            name: "Веб-технології та веб-дизайн",
                            teacher: 'Карашецький В.П.',
                            link: 'https://meet.google.com/euf-fgxw-rpr',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Вівторок',
                        lessonInfo: {
                            name: "Моделювання систем",
                            teacher: 'Крошний І.М.',
                            link: 'https://meet.google.com/qhq-akrw-pdr',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Cереда',
                        lessonInfo: {
                            name: "Технол. прог-ня та створ. прог. продуктів",
                            teacher: 'Різник О.Я.',
                            link: 'https://us05web.zoom.us/j/8980159994?pwd=NHdsL3BVemFrWWdvZjJyVHhnV3gzZz09',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Четвер',
                        lessonInfo: {
                            name: "Моделювання систем",
                            teacher: 'Сінкевич О.В.',
                            link: 'https://meet.google.com/rki-wfdd-umk',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'П\'ятниця',
                        lessonInfo:  {
                            name: "Управління ІТ-проектами",
                            teacher: 'Яцишин С.І.',
                            link: 'https://meet.google.com/nnd-gtfc-muk',
                            type: 'Лабораторне'
                        }
                    },
                ],
            },
            {
                time: '14:30',
                lessons: [
                    {
                        day: 'Понеділок',
                        lessonInfo: null
                    },
                    {
                        day: 'Вівторок',
                        lessonInfo: null
                    },
                    {
                        day: 'Cереда',
                        lessonInfo: {
                            name: "Системний аналіз",
                            teacher: 'Олянишен Т.В.',
                            link: 'https://meet.google.com',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'Четвер',
                        lessonInfo: {
                            name: "Технол. прог-ня та створ. прог. продуктів",
                            teacher: 'Різник О.Я.',
                            link: 'https://us05web.zoom.us/j/8980159994?pwd=NHdsL3BVemFrWWdvZjJyVHhnV3gzZz09',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'П\'ятниця',
                        lessonInfo: null
                    },
                ],
            },
            {
                time: '16:20',
                lessons: [
                    {
                        day: 'Понеділок',
                        lessonInfo: null
                    },
                    {
                        day: 'Вівторок',
                        lessonInfo: null
                    },
                    {
                        day: 'Cереда',
                        lessonInfo: {
                            name: "Методи та засоби ООАП",
                            teacher: 'Опришко М.І.',
                            link: 'https://meet.google.com/lookup/fjbq72rz64',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'Четвер',
                        lessonInfo: null
                    },
                    {
                        day: 'П\'ятниця',
                        lessonInfo: null
                    },
                ],
            }
        ]
    }

export const DominatorLessonsSchedule_KN_31_2: IGroup =
    {
        group: 'КН-31/2',
        isNumerator:false,
        schedule: [{
            time: '8:30',
            lessons: [
                {
                    day: 'Понеділок',
                    lessonInfo: null
                },
                {
                    day: 'Вівторок',
                    lessonInfo: {
                        name: "Програмування мобільних систем",
                        teacher: 'Сало М.Ф.',
                        link: 'https://meet.google.com/mab-hbgx-eyq',
                        type: 'Лекція'
                    }

                },
                {
                    day: 'Cереда',
                    lessonInfo: null
                },
                {
                    day: 'Четвер',
                    lessonInfo: null
                },
                {
                    day: 'П\'ятниця',
                    lessonInfo: null
                },
            ],
        },
            {
                time: '10:20',
                lessons: [
                    {
                        day: 'Понеділок',
                        lessonInfo: {
                            name: "Системний аналіз",
                            teacher: 'Олянишен Т.В.',
                            link: 'https://meet.google.com',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Вівторок',
                        lessonInfo: {
                            name: "ШНМ",
                            teacher: 'Шиманський В.М.',
                            link: 'https://meet.google.com/aqh-uymy-kia',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Cереда',
                        lessonInfo: {
                            name: "Веб-технології та веб-дизайн",
                            teacher: 'Яркун В.І.',
                            link: 'https://meet.google.com/tkb-nsfk-bvs',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'Четвер',
                        lessonInfo: {
                            name: "ШНМ",
                            teacher: 'Розумовський М.П.',
                            link: 'https://meet.google.com/beb-kffp-pif',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'П\'ятниця',
                        lessonInfo:  {
                            name: 'Програмув. мобільних систем',
                            teacher: 'Івасюк Р.В.',
                            link: 'https://meet.google.com/yar-vkvb-vqm',
                            type: 'Лабораторне'
                        }
                    },
                ],
            },
            {
                time: '12:10',
                lessons: [
                    {
                        day: 'Понеділок',
                        lessonInfo:  {
                            name: "Веб-технології та веб-дизайн",
                            teacher: 'Карашецький В.П.',
                            link: 'https://meet.google.com/euf-fgxw-rpr',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Вівторок',
                        lessonInfo: {
                            name: "Моделювання систем",
                            teacher: 'Крошний І.М.',
                            link: 'https://meet.google.com/qhq-akrw-pdr',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Cереда',
                        lessonInfo: {
                            name: "Управління ІТ-проектами",
                            teacher: 'Яцишин С.І.',
                            link: 'https://meet.google.com/nnd-gtfc-muk',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Четвер',
                        lessonInfo: {
                            name: "Моделювання систем",
                            teacher: 'Сінкевич О.В.',
                            link: 'https://meet.google.com/rki-wfdd-umk',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'П\'ятниця',
                        lessonInfo:  {
                            name: "Управління ІТ-проектами",
                            teacher: 'Яцишин С.І.',
                            link: 'https://meet.google.com/nnd-gtfc-muk',
                            type: 'Лабораторне'
                        }
                    },
                ],
            },
            {
                time: '14:30',
                lessons: [
                    {
                        day: 'Понеділок',
                        lessonInfo: null
                    },
                    {
                        day: 'Вівторок',
                        lessonInfo: null
                    },
                    {
                        day: 'Cереда',
                        lessonInfo: {
                            name: "Системний аналіз",
                            teacher: 'Олянишен Т.В.',
                            link: 'https://meet.google.com',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'Четвер',
                        lessonInfo: {
                            name: "Технол. прог-ня та створ. прог. продуктів",
                            teacher: 'Різник О.Я.',
                            link: 'https://us05web.zoom.us/j/8980159994?pwd=NHdsL3BVemFrWWdvZjJyVHhnV3gzZz09',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'П\'ятниця',
                        lessonInfo: null
                    },
                ],
            },
            {
                time: '16:20',
                lessons: [
                    {
                        day: 'Понеділок',
                        lessonInfo: null
                    },
                    {
                        day: 'Вівторок',
                        lessonInfo: null
                    },
                    {
                        day: 'Cереда',
                        lessonInfo: {
                            name: "Методи та засоби ООАП",
                            teacher: 'Опришко М.І.',
                            link: 'https://meet.google.com/lookup/fjbq72rz64',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'Четвер',
                        lessonInfo: null
                    },
                    {
                        day: 'П\'ятниця',
                        lessonInfo: null
                    },
                ],
            }
        ]
    }

export const NumeratorLessonsSchedule_KN_31_1: IGroup =
    {
        group: 'КН-31/1',
        isNumerator:true,
        schedule: [{
            time: '8:30',
            lessons: [
                {
                    day: 'Понеділок',
                    lessonInfo: null
                },
                {
                    day: 'Вівторок',
                    lessonInfo: null

                },
                {
                    day: 'Cереда',
                    lessonInfo:  {
                        name: "Технол. прог-ня та створ. прог. продуктів",
                        teacher: 'Різник О.Я.',
                        link: 'https://us05web.zoom.us/j/8980159994?pwd=NHdsL3BVemFrWWdvZjJyVHhnV3gzZz09',
                        type: 'Лабораторне'
                    }
                },
                {
                    day: 'Четвер',
                    lessonInfo:  {
                        name: "Веб-технології та веб-дизайн",
                        teacher: 'Яркун В.І.',
                        link: 'https://meet.google.com/tkb-nsfk-bvs',
                        type: 'Лабораторне'
                    }
                },
                {
                    day: 'П\'ятниця',
                    lessonInfo: null
                },
            ],
        },
            {
                time: '10:20',
                lessons: [
                    {
                        day: 'Понеділок',
                        lessonInfo: null
                    },
                    {
                        day: 'Вівторок',
                        lessonInfo: {
                            name: "Методи та засоби ООАП",
                            teacher: 'Соколовський Я.І.',
                            link: 'https://us02web.zoom.us/j/8894953167?pwd=dmZQNW9zR1lVV3pSaU5wd1RWVUgyUT09',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Cереда',
                        lessonInfo: {
                            name: "Системний аналіз",
                            teacher: 'Олянишен Т.В.',
                            link: 'https://meet.google.com',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'Четвер',
                        lessonInfo: {
                            name: "Моделювання систем",
                            teacher: 'Сінкевич О.В.',
                            link: 'https://meet.google.com/rki-wfdd-umk',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'П\'ятниця',
                        lessonInfo:  {
                            name: "Методи та засоби ООАП",
                            teacher: 'Опришко М.І.',
                            link: 'https://meet.google.com/lookup/fjbq72rz64',
                            type: 'Лабораторне'
                        }
                    },
                ],
            },
            {
                time: '12:10',
                lessons: [
                    {
                        day: 'Понеділок',
                        lessonInfo:  {
                            name: "Веб-технології та веб-дизайн",
                            teacher: 'Карашецький В.П.',
                            link: 'https://meet.google.com/euf-fgxw-rpr',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Вівторок',
                        lessonInfo: {
                            name: "Моделювання систем",
                            teacher: 'Крошний І.М.',
                            link: 'https://meet.google.com/qhq-akrw-pdr',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Cереда',
                        lessonInfo: {
                            name: "Технол. прог-ня та створ. прог. продуктів",
                            teacher: 'Різник О.Я.',
                            link: 'https://us05web.zoom.us/j/8980159994?pwd=NHdsL3BVemFrWWdvZjJyVHhnV3gzZz09',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Четвер',
                        lessonInfo: {
                            name: "ШНМ",
                            teacher: 'Розумовський М.П.',
                            link: 'https://meet.google.com/beb-kffp-pif',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'П\'ятниця',
                        lessonInfo:   {
                            name: 'Програмув. мобільних систем',
                            teacher: 'Сало М.Ф.',
                            link: 'https://meet.google.com/mab-hbgx-eyq',
                            type: 'Лабораторне'
                        }
                    },
                ],
            },
            {
                time: '14:30',
                lessons: [
                    {
                        day: 'Понеділок',
                        lessonInfo: null
                    },
                    {
                        day: 'Вівторок',
                        lessonInfo: null
                    },
                    {
                        day: 'Cереда',
                        lessonInfo: {
                            name: "Управління ІТ-проектами",
                            teacher: 'Яцишин С.І.',
                            link: 'https://meet.google.com/nnd-gtfc-muk',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'Четвер',
                        lessonInfo: null
                    },
                    {
                        day: 'П\'ятниця',
                        lessonInfo: null
                    },
                ],
            },
            {
                time: '16:20',
                lessons: [
                    {
                        day: 'Понеділок',
                        lessonInfo: null
                    },
                    {
                        day: 'Вівторок',
                        lessonInfo: null
                    },
                    {
                        day: 'Cереда',
                        lessonInfo: null
                    },
                    {
                        day: 'Четвер',
                        lessonInfo: null
                    },
                    {
                        day: 'П\'ятниця',
                        lessonInfo: null
                    },
                ],
            }
        ]
    }

export const DominatorLessonsSchedule_KN_31_1: IGroup =
    {
        group: 'КН-31/1',
        isNumerator:false,
        schedule: [{
            time: '8:30',
            lessons: [
                {
                    day: 'Понеділок',
                    lessonInfo: null
                },
                {
                    day: 'Вівторок',
                    lessonInfo: {
                        name: "Програмування мобільних систем",
                        teacher: 'Сало М.Ф.',
                        link: 'https://meet.google.com/mab-hbgx-eyq',
                        type: 'Лекція'
                    }

                },
                {
                    day: 'Cереда',
                    lessonInfo: {
                        name: "Технол. прог-ня та створ. прог. продуктів",
                        teacher: 'Різник О.Я.',
                        link: 'https://us05web.zoom.us/j/8980159994?pwd=NHdsL3BVemFrWWdvZjJyVHhnV3gzZz09',
                        type: 'Лабораторне'
                    }
                },
                {
                    day: 'Четвер',
                    lessonInfo: {
                        name: "Веб-технології та веб-дизайн",
                        teacher: 'Яркун В.І.',
                        link: 'https://meet.google.com/tkb-nsfk-bvs',
                        type: 'Лабораторне'
                    }
                },
                {
                    day: 'П\'ятниця',
                    lessonInfo: null
                },
            ],
        },
            {
                time: '10:20',
                lessons: [
                    {
                        day: 'Понеділок',
                        lessonInfo: {
                            name: "Системний аналіз",
                            teacher: 'Олянишен Т.В.',
                            link: 'https://meet.google.com',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Вівторок',
                        lessonInfo: {
                            name: "ШНМ",
                            teacher: 'Шиманський В.М.',
                            link: 'https://meet.google.com/aqh-uymy-kia',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Cереда',
                        lessonInfo: {
                            name: "Системний аналіз",
                            teacher: 'Олянишен Т.В.',
                            link: 'https://meet.google.com',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'Четвер',
                        lessonInfo: {
                            name: "Моделювання систем",
                            teacher: 'Сінкевич О.В.',
                            link: 'https://meet.google.com/rki-wfdd-umk',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'П\'ятниця',
                        lessonInfo:  {
                            name: "Методи та засоби ООАП",
                            teacher: 'Опришко М.І.',
                            link: 'https://meet.google.com/lookup/fjbq72rz64',
                            type: 'Лабораторне'
                        }
                    },
                ],
            },
            {
                time: '12:10',
                lessons: [
                    {
                        day: 'Понеділок',
                        lessonInfo:  {
                            name: "Веб-технології та веб-дизайн",
                            teacher: 'Карашецький В.П.',
                            link: 'https://meet.google.com/euf-fgxw-rpr',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Вівторок',
                        lessonInfo: {
                            name: "Моделювання систем",
                            teacher: 'Крошний І.М.',
                            link: 'https://meet.google.com/qhq-akrw-pdr',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Cереда',
                        lessonInfo: {
                            name: "Управління ІТ-проектами",
                            teacher: 'Яцишин С.І.',
                            link: 'https://meet.google.com/nnd-gtfc-muk',
                            type: 'Лекція'
                        }
                    },
                    {
                        day: 'Четвер',
                        lessonInfo:  {
                            name: "ШНМ",
                            teacher: 'Розумовський М.П.',
                            link: 'https://meet.google.com/beb-kffp-pif',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'П\'ятниця',
                        lessonInfo:  {
                            name: 'Програмув. мобільних систем',
                            teacher: 'Сало М.Ф.',
                            link: 'https://meet.google.com/mab-hbgx-eyq',
                            type: 'Лабораторне'
                        }
                    },
                ],
            },
            {
                time: '14:30',
                lessons: [
                    {
                        day: 'Понеділок',
                        lessonInfo: null
                    },
                    {
                        day: 'Вівторок',
                        lessonInfo: null
                    },
                    {
                        day: 'Cереда',
                        lessonInfo: {
                            name: "Управління ІТ-проектами",
                            teacher: 'Яцишин С.І.',
                            link: 'https://meet.google.com/nnd-gtfc-muk',
                            type: 'Лабораторне'
                        }
                    },
                    {
                        day: 'Четвер',
                        lessonInfo: null
                    },
                    {
                        day: 'П\'ятниця',
                        lessonInfo: null
                    },
                ],
            },
            {
                time: '16:20',
                lessons: [
                    {
                        day: 'Понеділок',
                        lessonInfo: null
                    },
                    {
                        day: 'Вівторок',
                        lessonInfo: null
                    },
                    {
                        day: 'Cереда',
                        lessonInfo: null
                    },
                    {
                        day: 'Четвер',
                        lessonInfo: null
                    },
                    {
                        day: 'П\'ятниця',
                        lessonInfo: null
                    },
                ],
            }
        ]
    }


export const AllGroupsNumeratorSchedule : IGroup[]=[NumeratorLessonsSchedule_KN_31_2, NumeratorLessonsSchedule_KN_31_1];

export const AllGroupsDominatorSchedule : IGroup[]=[DominatorLessonsSchedule_KN_31_2, DominatorLessonsSchedule_KN_31_1];

